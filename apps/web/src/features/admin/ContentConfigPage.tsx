...
function PanelsAndMarkersTab() {
  ...
  async function load() {
    const [p, m] = await Promise.all([
      ...
      apiFetch<MarkerRow[]>('/panels/markers'),
    ]);
    setPanels(p);
    setMarkers(m);
  }
  ...
  async function createPanel() {
    try {
      await apiFetch('/panels', { method: 'POST', body: JSON.stringify({ key: newPanelKey, name: newPanelName }) });
      setNewPanelKey('');
      setNewPanelName('');
          ...
          <div className="mt-4 flex flex-col gap-2">
            {panel.markers.map((pm) => (
              <div key={pm.id} className="flex flex-wrap items-center gap-3 border-b border-taupe pb-2 last:border-b-0">
                <span className="flex-1 text-sm text-espresso">{pm.marker.name}</span>
                <input
                  type="number"
          ...
          <div className="mt-4 flex flex-wrap items-end gap-3">
            <div className="min-w-48">
              ...
              >
                ...
                {markers
                  .filter((m) => !panel.markers.some((pm) => pm.markerId === m.id))
                  .map((m) => (
  ...
  return (
    <div className="flex flex-col gap-6">
      <Card className="max-w-md">
        <Select label="Marker" name="explanationMarker" value={selectedId} onChange={(e) => setSelectedId(e.target.value)}>
          <option value="">Select a marker…</option>
          {markers.map((m) => (
            <option key={m.id} value={m.id}>
              {m.name}
