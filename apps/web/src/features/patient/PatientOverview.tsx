...
import { formatLongDate, formatRelativeDate, type ChangeItem, type PatientOverview as Overview } from '../../lib/patientPortal';
    ...
    <Link to={`/markers/${change.markerId}`} className="rounded-card">
      <Card interactive className="flex h-full flex-col">
        ...
        <p className="mt-2 text-xs text-espresso/70">
          Compared with {formatLongDate(change.previousDate)}
        </p>
        <div className="mt-4">
      ...
      <header>
        ...
        {data.lastTestedDate ? (
          <p className="mt-5 max-w-2xl text-lg leading-relaxed text-espresso">
            ...
            <span className="font-medium">{formatLongDate(data.lastTestedDate)}</span> — {formatRelativeDate(data.lastTestedDate)}.
                  ...
                  <Link to={`/markers/${item.markerId}`} className="block rounded-card">
                    <Card interactive>
                      ...
                      <p className="mt-4 text-xs text-espresso/70">
                        {item.panelName} · {formatLongDate(item.sampleDate)}
        ...
        <section aria-labelledby="latest-heading">
          ...
          <Card className="mt-8">
            <div className="flex flex-wrap items-start justify-between gap-6">
              <div>
                <p className="eyebrow mb-2">{formatLongDate(data.latest.sampleDate)}</p>
