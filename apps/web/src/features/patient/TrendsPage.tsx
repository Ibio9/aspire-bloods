...
import { formatLongDate, type MarkerRow, type TrendSeries } from '../../lib/patientPortal';
                      ...
                      <Link key={s.markerId} to={`/markers/${s.markerId}`} className="rounded-card">
                        <Card interactive className="h-full !p-5 sm:!p-6">
                          ...
                          <p className="mt-1 text-xs text-espresso/70">
                            {formatLongDate(first.sampleDate)} to {formatLongDate(last.sampleDate)} · {statusLabel(last.status)}
                          </p>
                        </Card>
                      </Link>
                    );
