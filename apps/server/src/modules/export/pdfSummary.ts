...
export async function generateAspireSummaryPdf(reportId: string): Promise<Buffer> {
  ...
  // Body
  doc.fontSize(13).font('Helvetica-Bold').fillColor(ESPRESSO).text(`${panelLabel} — results summary`);
  doc.fontSize(10).font('Helvetica').moveDown(0.5);
  doc.text(`Dear ${profile?.firstName ?? 'Patient'},`);
  doc.moveDown(0.5);
  doc.text(
    `Please find below a summary of your ${panelLabel} results, from a sample taken on ${report.sampleDate.toLocaleDateString('en-GB')}.`,
  );
  doc.moveDown(1);
