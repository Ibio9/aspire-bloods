...
- **`ReferenceRange` holds two different things**: the catalogue of suggested fallbacks, and one row per result ever materialised recording what that patient's laboratory printed. The table therefore grows by one row per result for ever, and `resolveReferenceRange()` is handed all of them. It is not patient-facing — every displayed range comes off the result — but the range SUGGESTED to an admin at verify time is chosen from among them, and the fix is a schema change to a clinical data path. Since August 2026 the choice is at least deterministic: specificity first, then provenance tier, rather than whatever order Postgres returned. See `docs/audits/reference-ranges.md`.
