...
test.describe('marker names', () => {
  ...
  test('never break mid-word on the What’s changed cards, and those cards fit their content', async ({ browser }) => {
     ...
     * The complaint that produced `.card-row` is that a row of cards at visibly
     * different heights reads as a layout that did not finish, and the empty
     * half-card this test was written against is fixed the other way: the card
     * is a flex column, so the slack falls between its blocks.
     *
     * What is measured instead is the claim the class actually makes. Cards are
     * grouped by their painted TOP rather than by index, because how many fit
     * on a row is a function of the viewport and this test should not have to
     * know the column count to check the invariant.
     */
