...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 *  AND THE WARMTH CAME BACK, WITHOUT THE PAGE COMING BACK WITH IT (Aug 2026)
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * THE COMPLAINT, and it is a measurement rather than a preference: light mode
 * had drifted to a flat cool grey theme sitting beside a rich dark one. That is
 * what the four neutral hexes above produced, and it is not what the product is
 * — the register is a clinic, warm and quiet, not a console.
 *
 * ── WHAT MOVED, AND WHAT DELIBERATELY DID NOT ──────────────────────────────
 *
 * THREE of the four went warm. The fourth — the INK — did not go back to the
 * old `#423c36`, and that single restraint is what keeps this from being a
 * revert. `nightBase` is the ink taken most of the way to black, so the ink
 * decides whether the DARK PAGE is a near-black or a brown, and a warm brown
 * ink at 26% luminance is exactly what produced the viewport everybody
 * rejected. It is a warm NEAR-BLACK instead: r > g > b, so every surface
 * derived from it inherits the warmth, and #191512 is dark enough that the page
 * still reads black at a glance and warm only on inspection.
 *
 *   accent    #8A5E45  A warm bronze. It carries WHITE at 5.57:1 in light,
 *                      which is what sets its darkness — the same constraint
 *                      the slate answered to, met by a hue that belongs to the
 *                      rest of the page.
 *   ink       #191512  Warm near-black. Body text in light, and the seed of
 *                      every dark surface. ⚠ NOT the old espresso: see above.
 *   surface   #E4DED1  Cream. ⚠ ITS LUMINANCE IS THE LOAD-BEARING PART: a card
 *                      is this taken 90% to white, so the page→card step is the
 *                      gap between this and white and nothing else. #E4DED1
 *                      lands that step at 1.30:1, which is where it has always
 *                      been; a lighter cream collapses the whole light ladder
 *                      (the note on the neutral surface above records that
 *                      failure at #F3F4F6, and it is hue-independent).
 *   border    #C9BCA9  Taupe.
 *
 * ── WHAT THIS DOES TO DARK, WHICH IS THE HALF WORTH WATCHING ───────────────
 *
 * `nightLift` points at the border, so the border's hue is the hue of every
 * raised dark surface — the card, the hover state, the vellum. A warm taupe
 * therefore warms every dark surface WITHOUT touching the page, which is the
 * separation the retheme note above spent its whole length arguing for and is
 * the only reason it can be had both ways. Measured, dark: page #0F0D0B, card
 * #24201C, and the page→card ratio is unchanged.
 *
 * ── THE STATUS COLOURS ARE STILL UNTOUCHED ─────────────────────────────────
 *
 * `statusHue` is pinned as literals and `PLOT_SURFACE` is pinned at its own
 * luminance, so the five band fills, every contrast in `BAND_CONTRAST` and the
 * whole clinical layer are arithmetically identical either side of this. The
 * SOLVED block moved, because a wash is mixed from the card and a label is
 * solved against the page — and every value in it is regenerated by
 * `solveTokens()` rather than picked.
 */
export const brand = {
  /** THE ACCENT. Buttons, focus, active nav, links. Warm bronze again. */
  bronze: '#8A5E45',
  /** THE INK. Body text in light, and the seed of every dark surface. Warm near-black. */
  espresso: '#191512',
  /** THE SURFACE. The light page and the base of every light card. Cream. */
  cream: '#E4DED1',
  /** THE BORDER. Every hairline. Taupe. */
  taupe: '#C9BCA9',
  white: '#ffffff',
} as const;
 ...
 */
/**
 * ── AND THE QUIET ONE WENT WARM AGAIN WITH THE PALETTE (Aug 2026) ──────────
 *
 * `slate` holds #6B4260 — a muted plum. The KEY did not change and that is the
 * same decision `bronze` / `espresso` / `cream` / `taupe` are: it is a ROLE NAME
 * ("the quiet second accent") with a historical spelling, it is a Tailwind
 * colour family, a CSS custom property prefix and a name in several specs, and
 * renaming it is a mechanical sweep with no visual result on a change whose
 * entire risk is visual.
 *
 * The VALUE moved because its whole job did. It is the FILL light in light mode,
 * and light mode's page is cream again — a cool blue-slate over cream
 * desaturates rather than tints, which is precisely the "flat grey theme"
 * complaint arriving through the back door. A plum is red-dominant like the
 * bronze key, so the two read as two lamps in one room rather than as two
 * brands, and at ~313° it is 69° from the key, comfortably past the 20° the
 * two-sources test asks for.
 */
export const accent = {
  teal: '#2A6C74',
  /** THE QUIET ONE. A muted plum; the key is a role name, not a colour name. */
  slate: '#6B4260',
} as const;
...
const SOLVED: Record<'light' | 'dark', SolvedTokens> = {
  ...
  light: {
    ...
    wash: { green: '#dbe4d2', olive: '#e6e6ce', yellow: '#f1e7ca', orange: '#f0dfcc', red: '#ecd3ce' },
    track: { green: '#a0bb8b', olive: '#bfbf80', yellow: '#ddc376', orange: '#dcab7b', red: '#d18b80' },
    label: { green: '#3f5628', yellow: '#594613', red: '#903223' },
    bound: '#af9c80',
  },
  dark: {
    line: { green: '#669442', olive: '#939221', yellow: '#bf8f00', orange: '#d07a29', red: '#e06452' },
    wash: { green: '#2d3527', olive: '#32321f', yellow: '#362e16', orange: '#3e3022', red: '#453431' },
    track: { green: '#40582c', olive: '#4e4e0c', yellow: '#584201', orange: '#73461a', red: '#8c534a' },
    label: { green: '#80ae5b', yellow: '#bf8f00', red: '#f97c6a' },
    bound: '#6f5e46',
  },
};
...
function buildThemeTokens(mode: 'light' | 'dark'): Record<string, string> {
   ...
   */
  /**
   * ── AND IT IS DERIVED AGAIN RATHER THAN NAMED (Aug 2026) ─────────────────
   *
   * It was two literal hexes, a cool white-blue and a cool mid-grey, and it was
   * the one place in this file that named a hue rather than deriving one — so
   * it was also the one thing a change of palette could not reach. That is
   * exactly what happened twice: a gold glow survived the neutral retheme, and
   * a blue-white glow survived the warm one, each time as the most visible
   * contradiction on the screen.
   *
   * Both are the ACCENT'S OWN HUE ANGLE now, at a solved lightness, so a future
   * change to `brand.bronze` carries the ambient light with it. No new hue by
   * construction: `reHsl` keeps the angle and moves nothing else.
   *
   * THE TWO GO IN OPPOSITE DIRECTIONS, which is the same inversion the trend
   * chart's spark halo makes. On near-black a glow is LIGHT ADDED and has to be
   * far lighter than the page; on cream a pale warm anything measures about
   * 1.02:1 and what reads as warmth is the page taken slightly DOWN.
   */
  const glowPrimary = dark ? reHsl(brand.bronze, 0.62, 0.74) : mix(brand.bronze, brand.cream, 0.4);
  /**
   * ── THE FILL IS TEAL IN DARK AND PLUM IN LIGHT, AND THE SPLIT IS MEASURED ─
   *
   * The pair went to teal-in-both-themes when the key light was a cool
   * blue-grey: a blue key beside a blue-slate fill measured **4° apart in hue**,
   * and two lights 4° apart are one light with a wide falloff, which is the
   * exact failure a second source exists to avoid. That reading was right about
   * the fault and it was a fault of the KEY.
   *
   * With a warm key the constraint inverts, and the old reason light differed is
   * true again: a cool hue over CREAM desaturates rather than tints, so a teal
   * fill on this page is a grey corner rather than a second lamp. Light takes
   * the plum (69° off the key, red-dominant like it, so the two read as two
   * lamps in one room); dark keeps the teal, where there is no cream to
   * desaturate against and a cool counter-light is what gives a near-black page
   * its depth. `tokenContrast.test.ts` asserts the separation per theme rather
   * than the hue, so this is checked rather than asserted.
   */
  const glowSecondary = dark ? mix(accent.teal, '#7fd8d0', 0.62) : accent.slate;
  /**
  ...
  /**
   * 0.317 → 0.37 → 0.33, and every one of those moves is the SAME solve re-run
   * against a different border tone rather than three opinions about a hairline.
   * It is squeezed from both sides, which is why it is swept rather than nudged:
   * drawn at 55% over a band it has to mark the boundary there (>1.6:1 on every
   * fill and on the optimal narrowing) and stay visible as a rule on the DARK
   * key swatches (>1.3:1). Those pull in opposite directions, and the neutral
   * tone the line is derived from moves both at once.
   *
   * Swept at 0.01 across 0.28–0.52 against the warm taupe: below 0.32 the dark
   * green swatch fails (1.29:1 at 0.31 — a boundary you cannot see on the one
   * surface where a boundary must survive greyscale), and at 0.35 and above the
   * boundary itself drops under 1.6 on the heaviest band. The window is
   * 0.32–0.34 and this is the middle of it: boundary 1.641–1.964, swatch 1.411.
   *
   * ⚠ THE WINDOW MOVED BECAUSE THE TAUPE IS FAR MORE CHROMATIC THAN THE NEUTRAL
   * ONE WAS (23% saturation against 12%), and a more chromatic line at the same
   * lightness carries more weight against a coloured band. Re-sweep this rather
   * than nudging it whenever `brand.taupe` moves.
   */
  out['--c-chart-reference-edge'] = reLightness(brand.taupe, 0.33, ownSaturation(brand.taupe));
  /**
...
export const GLASS = {
   ...
   * ── 20px → 28px (Aug 2026) ───────────────────────────────────────────────
   *
   * Same measurement, same conclusion, one step further along it: what a
   * backdrop filter costs is the EXISTENCE of the pass and not the radius, so
   * every argument for a smaller number is an argument about a figure that does
   * not depend on it. 28 is where the frost stops being a diffusion of what is
   * behind and starts being an OBJECT with a thickness — the reading a pane
   * wants, and the one 20 was still short of where a card genuinely scrolls
   * underneath.
   */
  blur: '28px',
  /**
   ...
   */
  /**
   * ── 0.46 / 0.42 → 0.36 / 0.34 (Aug 2026), AND LIGHT MOVED FURTHEST ───────
   *
   * The frost barely read in light, and the arithmetic says why: a light pane's
   * fill and the page under it are 1.3:1 apart at most, so a pane at 0.46 was
   * painting nearly half of a colour that is very nearly the colour it is
   * painting over — almost all of the alpha spent, almost none of it visible.
   * In dark the same alpha buys a much larger step, which is why the two are no
   * longer the same number in the way they used to be.
   *
   * Under 0.4 the FILL is no longer what separates a pane at all: the streak,
   * the lit edge and the grain are, which is why all three went up in the same
   * pass. `tokenContrast.test.ts` measures the STREAKED pane rather than the
   * flat body for exactly this reason and still holds every label on it at AA.
   */
  panel: { light: 0.36, dark: 0.34 },
  /**
   ...
   */
  /**
   * ── ALL THREE WENT UP WITH THE FILL COMING DOWN (Aug 2026) ───────────────
   *
   * They are one decision. A pane at 0.36 has given up most of what the fill
   * used to do, and the three effects here are what it is given up FOR — a
   * surface you can see through that still has an edge, a highlight and a
   * thickness. Light gains the most because light had the least: a specular on
   * a near-white pane has very little room above it, so the streak has to be
   * strong to register at all, and the lit edge is doing the whole of the job
   * the fill used to share.
   */
  sheen: {
    peak: { light: 0.46, dark: 0.115 },
    edge: { top: { light: 0.68, dark: 0.2 }, right: { light: 0.92, dark: 0.33 } },
    grain: { light: 0.026, dark: 0.042 },
  },
} as const;
...
export const GLOW = {
   ...
   */
  /**
   * ── LIGHT WENT UP WITH THE PALETTE (Aug 2026): 0.10 / 0.095 → 0.13 / 0.11 ──
   *
   * The bound is unchanged and it is still the thing deciding these two: light's
   * pair both DARKEN the page where dark's ADD light to near-black, so every
   * point of alpha comes straight out of the espresso-on-cream ratio, and
   * `tokenContrast.test.ts` holds the pair to 15% of the bare page's own body
   * contrast.
   *
   * What moved is the headroom, not the appetite. Cream is a lower-luminance
   * page than the near-white grey it replaced, and both sources are now the
   * page's own family rather than colours crossing it, so the same visible
   * warmth costs measurably less contrast than it did. This is as high as the
   * budget goes, which is the same rule the old numbers were set by.
   */
  secondary: { light: 0.11, dark: 0.26 },
} as const;
...
export const chart = {
   ...
   *
   * `PLOT_SURFACE`, exposed so the ARC GAUGE can draw its ring on it. The
   * chart's own plot panel is gone; this is not that panel coming back, it is
   * the one surface `BAND_CONTRAST`, `BAND_FILL` and every ratio in the clinical
   * palette is measured on, made available to the one instrument still painting
   * those five fills.
   *
   * WHY THE GAUGE NEEDS IT, AND IT IS THE COMPLAINT STATED AS ARITHMETIC. The
   * five fills are byte-identical in the two themes — they always have been, and
   * `tokenContrast.test.ts` asserts it. What differs is what surrounds them: an
   * arc floating on a near-black card is the same gold read against a ground
   * five stops darker than the one it was solved for, and simultaneous contrast
   * does the rest. The gold is the hue that shows it worst, because a yellow's
   * apparent lightness is most of what identifies it as yellow.
   *
   * So the ring sits in a WELL of this tone in both themes, exactly as the range
   * bar's track did before the bar became an arc. Nothing about the five colours
   * changes; the ground stops changing under them.
   */
